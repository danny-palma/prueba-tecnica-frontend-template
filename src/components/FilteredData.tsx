...
export const FilteredData: React.FC<FilteredDataProps> = ({
  filteredData,
  currentPage,
  itemsPerPage,
  onLoadMore,
  ...
  const paginatedData = filteredData.slice(0, endIdx)
  ...
  return (
    <>
      {paginatedData.map((item, index: number) => (
        <div
          key={item.id}
          className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 shadow-sm hover:shadow-lg transform hover:-translate-y-1 transition-all duration-200"
        >
      ...
      {hasMore && (
        <div className="col-span-full flex justify-center py-8">
          ...
          >
            Cargar más ({paginatedData.length} de {filteredData.length})
          </button>
        </div>
      )}
    </>
